...
FLOAT = {10, 14, 15, 16, 17, 19, 21, 46, 521, 522, 523}
Dlow = np.array([
...
def _read_pyatmoslogger(filename: str | Path) -> LpmOutput:
    time = []
    data: dict = {}
...
def _read_lampedusa(filename: str | Path) -> LpmOutput:
    time = []
    data: dict = {}
...
def _read_raw(filename: str | Path) -> LpmOutput:
    time = []
    data: dict = {}
    with open(filename) as f:
        for line in f:
