...
from cloudnetpy.model_evaluation.products.advance_methods import apply_cirrus_filter
from cloudnetpy.model_evaluation.products.grid_methods import downsample_to_model_grid
...
def process_L3_day_product(
    ...
    *,
    ...
    product_obj = ObservationManager(product, product_file)
    model_obj = ModelManager(model_file, model, product)
    try:
        try:
            apply_cirrus_filter(model_obj, product_obj)
        except ValueError as e:
            logging.info(e)
        downsample_to_model_grid(model_obj, product_obj)
