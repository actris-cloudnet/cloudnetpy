...
class TestCopernicus2nc(Check):
    site_meta = {"name": "Chilbolton", "latitude": 50, "longitude": 104.5, "altitude": 50}
    ...
    def test_variables(self):
        assert np.isclose(self.nc.variables["radar_frequency"][:].data, 34.96)  # Hard coded
        assert np.all(np.isclose(self.nc.variables["zenith_angle"][:].data, -0.13, atol=0.1))
    ...
    def test_wrong_date_validation(self, tmp_path):
        ...
        with pytest.raises(ValidTimeStampError):
            copernicus.copernicus2nc(FILEPATH, test_path, self.site_meta, date="2021-01-03")

    def test_uuid_from_user(self, tmp_path):
        test_path = tmp_path / "uuid.nc"
        uuid_from_user = "kissa"
        uuid = copernicus.copernicus2nc(FILEPATH, test_path, self.site_meta, uuid=uuid_from_user)
