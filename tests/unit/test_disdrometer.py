...
class TestParsivelKenttarova(Check):
    date = "2025-11-18"
    temp_dir = TemporaryDirectory()
    temp_path = temp_dir.name + "/test.nc"
    filename = f"{SCRIPT_PATH}/data/parsivel/20251118_kenttarova_parsivel2.csv"
    site_meta = SITE_META
    uuid = disdrometer.parsivel2nc(filename, temp_path, site_meta, date=date)
...
class TestParsivelEriswilOld(Check):
    date = "2022-03-30"
    temp_dir = TemporaryDirectory()
    temp_path = temp_dir.name + "/test.nc"
    filename = f"{SCRIPT_PATH}/data/parsivel/CLOUDLAB_parsivel_20220330_20220331.nc"
    site_meta = SITE_META
    uuid = disdrometer.parsivel2nc(filename, temp_path, site_meta, date=date)
...
class TestThies7(Check):
    date = "2025-02-19"
    temp_dir = TemporaryDirectory()
    temp_path = temp_dir.name + "/test.nc"
    filename = f"{SCRIPT_PATH}/data/thies-lnm/rsa1_2025021900.txt"
    site_meta = SITE_META
    uuid = disdrometer.thies2nc(filename, temp_path, site_meta, date=date)
